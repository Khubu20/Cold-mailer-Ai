...
const ReferralEmailForm = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/generate-referral-email`, formData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        setError(response.data.error || 'Failed to generate referral email');
      }
    } catch (error) {
      ...
      if (error.response?.data?.error) {
        ...
        setError('Failed to generate referral email. Please try again.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <form onSubmit={handleSubmit} className="form">
      ...
      <div className="form-section">
        ...
        {inputType === 'text' ? (
          <textarea
            ...
            placeholder="✨ Paste the job description here... AI will extract company name, position, and job ID automatically!"
            rows="8"
      ...
      <div className="form-section">
        ...
        <textarea
          ...
          placeholder="💫 Example: Currently working as SDE Intern at Infosys, skilled in Java full-stack, React.js, JavaScript, graduating in 2025, available for immediate joining..."
          rows="5"
          className="text-input"
        />
      </div>
